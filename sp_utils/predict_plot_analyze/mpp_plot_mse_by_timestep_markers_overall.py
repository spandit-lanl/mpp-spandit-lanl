...
PLOTS = {
    "Uvelocity": ("mse_Uvelocity", "Uvelocity", "#ff7f0e", "o"),
    "Wvelocity": ("mse_Wvelocity", "Wvelocity", "#2ca02c", "s"),
    "density_maincharge": ("mse_density_maincharge", "density_maincharge", "#d62728", "^"),
    "av_density": ("mse_av_density", "av_density", "#1f77b4", "D"),
    "mse_avg_over_fields": ("mse_mean_fields", "mse_avg_over_fields", "#000000", "X"),
}

def overall_mse_all_fields_all_timesteps_all_sims(df: pd.DataFrame) -> float:
    """Return a single scalar: mean MSE over all fields, all timesteps, all simulations.

    Notes:
      - Uses the per-row mse_mean_fields column if present (preferred).
      - Otherwise computes mean across the 4 per-field MSE columns per row, then averages across rows.
      - This is an unweighted average across (sim_id, timestep) rows; each row's MSE is assumed
        to already be an average over spatial points.
    """
    if 'mse_mean_fields' in df.columns:
        return float(pd.to_numeric(df['mse_mean_fields'], errors='coerce').dropna().mean())
    # fallback: average the 4 per-field mse columns
    cols = ['mse_av_density','mse_Uvelocity','mse_Wvelocity','mse_density_maincharge']
    for c in cols:
        if c not in df.columns:
            raise ValueError(f"Missing required column for overall MSE: {c}")
    tmp = df[cols].apply(pd.to_numeric, errors='coerce')
    return float(tmp.mean(axis=1, skipna=True).dropna().mean())
...
def main():
    ...
    overall = overall_mse_all_fields_all_timesteps_all_sims(df)
    print(f"Overall MSE (all fields, all timesteps, all simulations): {overall:.6g}")
    ...
    for ax, key in layout:
        col, title, color, marker = PLOTS[key]
        ax.plot(agg["timestep"].values, agg[col].values, color=color, linewidth=1.8, linestyle=':', marker=marker, markersize=3.5)
