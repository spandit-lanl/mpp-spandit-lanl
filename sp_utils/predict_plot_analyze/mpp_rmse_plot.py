#!/usr/bin/env python3
"""mpp_rmse_plot.py (fixed)

Reads rmse_summary.csv produced by mpp_rmse_summary.py and produces:
...
def cum_plot_field(dfv: pd.DataFrame, field: str, var: int, out_dir: Path, dpi: int):
    ...
    plt.tight_layout(rect=[0, 0, 0.80, 1])
    plt.tight_layout()
    plt.savefig(out_dir / f"rmse_cum_field-{field}_var-{var}.pdf")
    plt.close()
...
def step_plot_allfields(step: pd.DataFrame, var: int, out_dir: Path, dpi: int):
    ...
    plt.xlabel("Target Timestep")
    plt.ylabel("RMSE")
    plt.title("RMSE — ALL FIELDS")
    plt.legend(loc="center left", bbox_to_anchor=(1.02, 0.5), fontsize=8, borderaxespad=0.0)
    plt.tight_layout(rect=[0, 0, 0.80, 1])
    plt.tight_layout()
    plt.savefig(out_dir / f"rmse_step_ALLFIELDS_var-{var}.pdf")
    plt.close()
...
def cum_plot_allfields(cum: pd.DataFrame, var: int, out_dir: Path, dpi: int):
    ...
    plt.tight_layout(rect=[0, 0, 0.80, 1])
    plt.tight_layout()
    plt.savefig(out_dir / f"rmse_cum_ALLFIELDS_var-{var}.pdf")
    plt.close()
