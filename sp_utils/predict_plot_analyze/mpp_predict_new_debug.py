...
def discover_timesteps(data_dir: str, series: str, suffix: str) -> Dict[str, List[int]]:
    ...
    for fn in os.listdir(data_dir):
        # DEBUG: verify regex matching for a known sim_id
        if fn.startswith(f"{series}_id02540_{suffix}_idx") and fn.endswith('.npz'):
            print(f"DEBUG discover saw fn={fn!r} match={bool(pat.match(fn))}", flush=True)
        m = pat.match(fn)
        if not m:
            continue
        sim = m.group("sim")
...
def main() -> None:
    ...
    if args.sim_ids_file is not None:
        ...
        print(f"DEBUG data_dir={data_dir} exists={os.path.isdir(data_dir)} n_files={len(os.listdir(data_dir))}", flush=True)
        print(f"DEBUG ts_map keys={len(ts_map)} has_02540={'02540' in ts_map}", flush=True)
        if len(ts_map) > 0:
            print(f"DEBUG ts_map sample keys={list(ts_map.keys())[:10]}", flush=True)
        ...
        for sim in sims:
            tlist = ts_map.get(sim, [])
            if not tlist:
                expected_pattern = f"{data_dir}/{series}_id{sim}_{args.suffix}_idx*.npz"
                print(f"[WARN] no files found for sim_id={sim} (repr={sim!r}) in_ts_map={sim in ts_map}; skipping. Expected pattern: {expected_pattern}", flush=True)
                continue
