...
def main():
    ...
    if not out_root.exists():
        print(f"[WARN] {os.path.basename(__file__)}: --out_root does not exist: {out_root}", file=sys.stderr)
        return
    ...
    if not scan_root.exists():
        print(f"[WARN] {os.path.basename(__file__)}: Scan root does not exist: {scan_root}", file=sys.stderr)
        return

    pred_paths = list(scan_root.rglob("pred.npz"))
    if not pred_paths:
        print(f"[WARN] {os.path.basename(__file__)}: No pred.npz found under {scan_root}; skipping.", file=sys.stderr)
        return
    ...
    for p in sorted(pred_paths):
        ...
        if meta is None:
            if args.strict_layout:
                print(f"[WARN] {os.path.basename(__file__)}: pred.npz path does not match expected layout: {p}; skipping this file.", file=sys.stderr)
                continue
            else:
                continue
    ...
    if not rows:
        print(f"[WARN] {os.path.basename(__file__)}: No matching pred.npz found for expt_id={args.expt_id} under {scan_root}; skipping.", file=sys.stderr)
        return
